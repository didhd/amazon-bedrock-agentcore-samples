    ...
    try:
        ...
        if agent_interactions:
            ...
            for i, interaction in enumerate(agent_interactions[-3:], 1):  # Show last 3
                ...
                interaction_preview = (
                    f"{interaction_str[:100]}..."
                    if len(interaction_str) > 100
                    else interaction_str
                )
        ...
        print(
            f"   🔧 3 tools (get_tables_information, current_time, execute_sql_query)"
        )
        ...
        async for event in stream:
            if (
                ...
                and "role" in event["message"]
                and event["message"]["role"] == "assistant"
            ):
                for content_item in event["message"]["content"]:
                    if (
                        "toolUse" in content_item
                        and "input" in content_item["toolUse"]
                        and content_item["toolUse"]["name"] == "execute_sql_query"
                    ):
                        yield f" {content_item['toolUse']['input']['description']}.\n\n"
                    elif (
                        "toolUse" in content_item
                        and "name" in content_item["toolUse"]
                        and content_item["toolUse"]["name"] == "get_tables_information"
                    ):
                        yield "\n\n"
                    elif (
                        "toolUse" in content_item
                        and "name" in content_item["toolUse"]
                        and content_item["toolUse"]["name"] == "current_time"
                    ):
                        yield "\n\n"
            elif "data" in event:
                yield event["data"]
        ...
        save_agent_interactions(
            session_id, prompt_uuid, starting_message_id, agent.messages
        )
